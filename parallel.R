...
beta_setup = 3 # (1) all signals 1; (2) all signals uniform (0.3,1) normalized; (3) IDS
...
regret_list  = foreach(i = 1:nrep, .packages = c("MASS", "sparsevb", "ebreg", "R6", "monomvn", "glmnet")) %dopar% {
  ...
  lasso.opt = Lasso.optimism$new(lambda= 0.5, tau=.5, p=p, K=K, T0=2)
  estc = ESTC$new(lambda=0.5, p=p, K=K, T0=n1)
  lints = LinTS$new(delta=0.01, epsilon=1/log(Th), p=p, K=K, error.var=sigma^2)
  vbts = VB.complexity.TS$new(p=p, K=K, u=1.001, T0=10, T1 = 25)
...
if (save_file == 1){
if(corr_model == 1){
filename = paste("regret", "d", p, "s", s, "K", K, "beta_setup", beta_setup, "corr", rho2,".csv", sep = "_")
}
if(corr_model == 2){
filename = paste("regret", "d", p, "s", s, "K", K, "beta_setup", beta_setup, "AR", phi,".csv", sep = "_")    
}
...
sp = df
plot(1:Th, sp$meanregret.VBTS, type = "l", ylim = c(0,500))
