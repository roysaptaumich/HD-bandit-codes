...
rho2 = 0.5
phi = 0.5
corr_model = 1 # 1: equicorrelation, 2: AR1(phi)
if(corr_model == 1){
  V = (sigma^2 - rho2) * diag(1, p) + rho2 * matrix(1, nrow=p, ncol=p)
}
if(corr_model == 2){
  e = abs(matrix(1:p -1, nrow = p, ncol = p, byrow = T) - (1:p -1))
  V = phi^e
}
beta_setup = 1 # (1) all signals 1; (2) all signals uniform (0, 0.3);  (3) IDS
...
if (beta_setup == 1){
  ...
  beta[idx] = runif(s, min=0.3, max=1)
...
regret_list  = foreach(i = 1:nrep, .packages = c("MASS", "sparsevb", "ebreg", "R6", "monomvn", "glmnet")) %dopar% {
  ...
  lasso.opt = Lasso.optimism$new(lambda= 1.5, tau=.2, p=p, K=K, T0=2)
  estc = ESTC$new(lambda=0.5, p=p, K=K, T0=n1)
  lints = LinTS$new(delta=0.01, epsilon=1/log(Th), p=p, K=K, error.var=sigma^2)
  vbts = VB.complexity.TS$new(p=p, K=K, lambda = 0.5 * log(p)^0.5, u=1.001, T0= 5, T1 = 1)
  ...
  #X_full =  matrix(runif(K*Th*p, min = -1, max = 1), ncol = p) 
  ...
  for (t in 1:Th){
    ...
    # LinTS
    start = Sys.time()
    a5 = 1 #lints$choose.action(t, t(X))
    ...
    # VB_TS
    start = Sys.time()
    a6 = try(vbts$choose.action(t, t(X)), T)
    #cat(class(a6))
    #cat(class(a6)=="numeric")
    if (class(a6) == 'try-error'){
      cat("nrep", i,"round",t, "error action choice VBTS","\n")
      a6 = sample(1:K,1)
    }
    chosen.mean = X[a6,] %*% beta
    vbts.mean_rewards[t] = chosen.mean
    vbts$update.beta((chosen.mean + error), X[a6,], t)
    #is_error = try(vbts$update.beta((chosen.mean + error), X[a6,], t))
    #if (class(is_error) == 'try-error'){
    #  cat("nrep", i, "round", t, "error in update VBTS", "\n")
    #}
    #else{
    ...
    # BLASSO_TS
    start = Sys.time()
    a8 = 1 #blassots$choose.action(t, t(X))
  ...
  regret.ebregts = cumsum(opt.mean_rewards) - cumsum(opt.mean_rewards) ## Ebreg is off
...
methods = c("Random", "LinUCB", "DRLasso", "Lasso-L1", "ESTC", "LinTS", "VBTS", "EBRegTS", "BLassoTS")
m = length(methods)
...
if (save_file == 1){
  if(corr_model == 1){
    filename = paste("new_regret", "d", p, "s", s, "K", K, "beta_setup", beta_setup, "corr", rho2,".csv", sep = "_")
  }
  if(corr_model == 2){
    filename = paste("new_regret", "d", p, "s", s, "K", K, "beta_setup", beta_setup, "AR", phi,".csv", sep = "_")    
  }
  file_path = paste("/home/user/Bayes_SLCB/R/",filename,sep="") # give proper file path
  write.csv(df,file =  file_path, row.names = F)
}


sp = df
plot(1:Th, sp$meanregret.VBTS, type = "l", ylim = c(0,1000))
lines(1:Th,sp$meanregret.Lasso_L1, col = "blue")

lines(1:Th,sp$meanregret.ESTC, col = "pink")
