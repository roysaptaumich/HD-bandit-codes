...
#install.packages("monomvn",repos = "https://cloud.r-project.org")
#install.packages("sparsevb",repos = "https://cloud.r-project.org")
#install.packages("ebreg",repos = "https://cloud.r-project.org")
#install.packages("glmnet",repos = "https://cloud.r-project.org")
#install.packages("functClust",repos = "https://cloud.r-project.org")
...
LinTS = R6Class("LinTS",
                public = list(
                  ...
                  choose.action = function(t, contexts){
                    mu.TS = mvrnorm(1, mu=self$mu.hat, Sigma=self$v2 * self$B.inv)
                   ...
                   public = list(
                    ...
                    update.beta = function(reward, chosen.context, t){
                      ...
                      if (t>= self$T0){
                        ...
                        RSS = mean((self$obs.rewards - self$chosen.contexts %*% self$beta)^2)
...
VB.complexity.TS = R6Class("VB.complexity.TS",
                           public = list(
                             ...
                             initialize = function(p, K, u=1.001, lambda=1, T0, T1, err.sd=1, max_iter = 1000  ,tol = 1e-05){
                               self$p = p
                               self$K = K
                               self$u = u
                               self$beta = numeric(p)
                             ...
                             choose.action = function(t, contexts){
                               if (t <= self$T0){
                                 if(t<=3){action = sample(self$K, 1)}
                                 else{
                                  lasso_cv = cv.glmnet(x = self$chosen.contexts, y = self$obs.rewards, 
                                                   family = "gaussian", intercept = F)
                                  lambda = lasso_cv$lambda.min
                                  beta_hat = lasso_cv$glmnet.fit$beta[, which(lasso_cv$lambda == lambda)]
                                 
                                  est = t(contexts) %*% beta_hat
                                  action = which.max(est) 
                                 }
                                 #action = sample(self$K, 1)
                               }else{
                             ...
                             update.beta = function(reward, chosen.context, t){
                               ...
                               q = 1 # nrow(self$chosen.contexts)
                               #q = 1 * I(q<=100) + q * I(q>100)
                               if (t >= self$T0){
                                 if (t %% self$T1 == self$T0+1){
                                   vb.sparse = svb.fit(
                                     self$chosen.contexts/sqrt(q),
                                     self$obs.rewards/sqrt(q),
                                     family = "linear",
                                     slab = "laplace",
                                     alpha = 1,
                                     beta = p^self$u,
                                     prior_scale = self$lambda,
                                     intercept = FALSE,
